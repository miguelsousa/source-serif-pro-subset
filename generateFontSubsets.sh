...
function generate_subsets {
	pyftsubset "original/SourceSerifPro-"$1".ttf" --output-file="subset/sourceserifpro-"$2"-webfont.ttf" --unicodes-file="codepoints.txt" --no-ignore-missing-unicodes --notdef-outline --glyph-names --name-IDs='*' --layout-features+=smcp,onum --drop-tables-=BASE --no-subset-tables+=BASE
}
